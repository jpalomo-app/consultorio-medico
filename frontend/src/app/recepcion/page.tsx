...
const CONFIG_TABS: { id:ConfigTab; label:string; icon:string }[] = [
  ...
  { id:"profesionales",   label:"Profesionales",  icon:"\U0001fa7a" },
  { id:"agendas",         label:"Agendas",        icon:"\U0001f4c5" },
  { id:"obras_sociales",  label:"Obras sociales", icon:"\U0001f3e5" },
];
...
export default function RecepcionPage() {
  ...
  const turnosFiltrados = useMemo(() =>
    turnos.filter(t => {
      ...
      return pasaEstado && pasaProf;
    }), [turnos, filtroEstado, filtroProfesional]);
            ...
            {vista === "turnos" && (
              <div className="flex items-center gap-1">
                ...
                <input type="date" value={fecha} onChange={e=>setFecha(e.target.value)}
                  className="px-2 py-1.5 rounded-lg bg-white/10 border border-white/10 text-xs text-white focus:outline-none focus:ring-2 focus:ring-brand-500 [color-scheme:dark]" />
                <button onClick={()=>setFecha(sumarDias(fecha,1))} className="w-8 h-8 rounded-lg bg-white/10 hover:bg-white/20 border border-white/10 text-white flex items-center justify-center transition-all">›</button>
              </div>
            )}
          </div>
            ...
            <div className="flex-1 min-w-0 space-y-5">
              ...
              {cargando ? (
                ...
                <div className="text-center py-16 text-gray-500">
                  <p className="text-4xl mb-3">\U0001f4c5</p>
